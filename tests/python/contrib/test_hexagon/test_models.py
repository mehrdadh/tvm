...
@pytest.mark.parametrize("enable_usmp", [False, True])
@tvm.testing.requires_hexagon
def test_mobilenet_aot_relax(hexagon_session: Session, aot_host_target, aot_target, enable_usmp):
    """Test mobilenet with aot executor"""
    dtype = "float32"
    onnx_model = get_mobilenet()
    ...
    target_hexagon = tvm.target.Target(aot_target, host=aot_host_target)
    input_name = "data"
    shape_dict = {input_name: data_in.shape}
    relay_mod, params = relay.frontend.from_onnx(onnx_model, shape_dict, freeze_params=True)
    inputs = {input_name: data_in}
    ...
    with tvm.transform.PassContext(config={"relax.usmp.enable": enable_usmp}):
        mod = build(relax_mod, target_hexagon)
